...
internal sealed class NumberFormatter
{
	...
	private int IntegerDigits => (_decPointPos <= 0) ? 1 : _decPointPos;

	private int DecimalDigits => (_digitsLen > _decPointPos) ? (_digitsLen - _decPointPos) : 0;

	private bool IsFloatingSource => _defPrecision == 15 || _defPrecision == 7;

	private bool IsZero => _digitsLen == 0;

	private bool IsZeroInteger => _digitsLen == 0 || _decPointPos <= 0;
	...
	private unsafe static uint FastToDecHex(int val)
	{
		...
		int num = val * 5243 >> 19;
		return (uint)(*(int*)((byte*)DecHexDigits + num * 4) << 8) | *(uint*)((byte*)DecHexDigits + (val - num * 100) * 4);
	}
	...
	private string FastIntegerToString(int value, IFormatProvider fp)
	{
		...
		{
			int num = value / 10000;
			FastAppendDigits(num, force: false);
			FastAppendDigits(value - num * 10000, force: true);
		}
		else
		{
			FastAppendDigits(value, force: false);
		}
	...
	private static void AppendNonNegativeNumber(StringBuilder sb, int v)
	{
		...
		do
		{
			...
			sb.Append((char)(0x30u | (uint)num2));
	...
	private void AppendExponent(NumberFormatInfo nfi, int exponent, int minDigits)
	{
		...
		if (exponent < 10)
		{
			...
			Append((char)(0x30u | (uint)exponent));
			return;
		}
		uint num = FastToDecHex(exponent);
		if (exponent >= 100 || minDigits == 3)
		{
			Append((char)(0x30u | (num >> 8)));
		}
		Append((char)(0x30u | ((num >> 4) & 0xFu)));
		Append((char)(0x30u | (num & 0xFu)));
	}
	...
	private void AppendOneDigit(int start)
	{
		...
		num >>= (start & 7) << 2;
		_cbuf[_ind++] = (char)(0x30u | (num & 0xFu));
	}
	...
	private unsafe void FastAppendDigits(int val, bool force)
	{
		...
		if (force || val >= 100)
		{
			...
			if (force || val >= 1000)
			{
				_cbuf[ind++] = (char)(0x30u | (uint)(num2 >> 4));
			}
			_cbuf[ind++] = (char)(0x30u | ((uint)num2 & 0xFu));
	...
	private void AppendDigits(int start, int end)
	{
		...
		while (true)
		{
			...
			_cbuf[--num] = (char)(0x30u | (num3 & 0xFu));
			switch (num2 - start)
			{
			case 8:
				_cbuf[--num] = (char)(0x30u | ((num3 >>= 4) & 0xFu));
				goto case 7;
			case 7:
				_cbuf[--num] = (char)(0x30u | ((num3 >>= 4) & 0xFu));
				goto case 6;
	...
	private void AppendDigits(int start, int end, StringBuilder sb)
	{
		...
		while (true)
		{
			...
			sb[--num] = (char)(0x30u | (num3 & 0xFu));
			switch (num2 - start)
			{
			case 8:
				sb[--num] = (char)(0x30u | ((num3 >>= 4) & 0xFu));
				goto case 7;
			case 7:
				sb[--num] = (char)(0x30u | ((num3 >>= 4) & 0xFu));
				goto case 6;
