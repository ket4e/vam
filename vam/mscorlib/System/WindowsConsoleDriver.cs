...
internal class WindowsConsoleDriver : IConsoleDriver
{
	...
	public bool NumberLock
	{
		get
		{
			short keyState = GetKeyState(144);
			return (keyState & 1) == 1;
		}
	}
	...
	public bool TreatControlCAsInput
	{
		...
		set
		{
			...
			bool flag = (mode & 1) == 0;
			if (flag == value || SetConsoleMode(mode: (!value) ? (mode + 1) : (mode & -2), handle: outputHandle))
			{
				return;
			}
	...
	public unsafe void MoveBufferArea(int sourceLeft, int sourceTop, int sourceWidth, int sourceHeight, int targetLeft, int targetTop, char sourceChar, ConsoleColor sourceForeColor, ConsoleColor sourceBackColor)
	{
		...
		fixed (void* buffer = &System.Runtime.CompilerServices.Unsafe.AsRef<void>(System.Runtime.CompilerServices.Unsafe.AsPointer(ref array[0])))
		{
