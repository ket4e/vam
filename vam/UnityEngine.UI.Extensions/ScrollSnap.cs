	...
	public void UpdateListItemPositions()
	{
		...
		if (num > 0)
		{
			...
			if (direction == ScrollDirection.Horizontal)
			{
				...
				{
					ref Vector3 reference = ref _pageAnchorPositions[i];
					reference = new Vector3(_listContainerMaxPosition - _itemSize * (float)i, _listContainerTransform.localPosition.y, _listContainerTransform.localPosition.z);
				}
			}
			else
			{
				...
				{
					ref Vector3 reference2 = ref _pageAnchorPositions[j];
					reference2 = new Vector3(_listContainerTransform.localPosition.x, _listContainerMinPosition + _itemSize * (float)j, _listContainerTransform.localPosition.z);
				}
			}
