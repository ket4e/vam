...
public sealed class BASS_PLUGININFO
{
	...
	internal BASS_PLUGININFO(IntPtr pluginInfoPtr)
	{
		...
		{
			bass_plugininfo bass_plugininfo2 = (bass_plugininfo)Marshal.PtrToStructure(pluginInfoPtr, typeof(bass_plugininfo));
			version = bass_plugininfo2.version;
			formatc = bass_plugininfo2.formatc;
			formats = new BASS_PLUGINFORM[formatc];
			ReadArrayStructure(formatc, bass_plugininfo2.formats);
		}
	}
	...
	private unsafe void ReadArrayStructure(int count, IntPtr p)
	{
		for (int i = 0; i < count; i++)
		{
			try
			{
				...
				p = new IntPtr((byte*)p.ToPointer() + Marshal.SizeOf(formats[i]));
			}
			catch
			{
				break;
