...
internal static class GDIPlus
{
	...
	public static void FromUnManagedMemoryToPointI(IntPtr prt, Point[] pts)
	{
		...
		{
			ref Point reference = ref pts[num2];
			reference = (Point)Marshal.PtrToStructure(ptr, typeof(Point));
			num2++;
	...
	public static void FromUnManagedMemoryToPoint(IntPtr prt, PointF[] pts)
	{
		...
		while (num2 < pts.Length)
		{
			ref PointF reference = ref pts[num2];
			reference = (PointF)Marshal.PtrToStructure(ptr, typeof(PointF));
			num2++;
			ptr = new IntPtr(ptr.ToInt64() + num);
		}
		Marshal.FreeHGlobal(prt);
	...
	public static void FromUnManagedMemoryToRectangles(IntPtr prt, RectangleF[] pts)
	{
		...
		while (num2 < pts.Length)
		{
			ref RectangleF reference = ref pts[num2];
			reference = (RectangleF)Marshal.PtrToStructure(ptr, typeof(RectangleF));
			num2++;
			ptr = new IntPtr(ptr.ToInt64() + num);
		}
		Marshal.FreeHGlobal(prt);
	...
	internal static void CheckStatus(Status status)
	{
		...
		{
			string text = global::Locale.GetText("Generic Error [GDI+ status: {0}]", status);
			throw new Exception(text);
		}
		case Status.InvalidParameter:
		{
			string text = global::Locale.GetText("A null reference or invalid value was found [GDI+ status: {0}]", status);
			throw new ArgumentException(text);
		}
		case Status.OutOfMemory:
		{
			string text = global::Locale.GetText("Not enough memory to complete operation [GDI+ status: {0}]", status);
			throw new OutOfMemoryException(text);
		}
		case Status.ObjectBusy:
		{
			string text = global::Locale.GetText("Object is busy and cannot state allow this operation [GDI+ status: {0}]", status);
			throw new MemberAccessException(text);
		}
		case Status.InsufficientBuffer:
		{
			string text = global::Locale.GetText("Insufficient buffer provided to complete operation [GDI+ status: {0}]", status);
			throw new InternalBufferOverflowException(text);
		}
		case Status.PropertyNotSupported:
		{
			string text = global::Locale.GetText("Property not supported [GDI+ status: {0}]", status);
			throw new NotSupportedException(text);
		}
		case Status.FileNotFound:
		{
			string text = global::Locale.GetText("Requested file was not found [GDI+ status: {0}]", status);
			throw new FileNotFoundException(text);
		}
		case Status.AccessDenied:
		{
			string text = global::Locale.GetText("Access to resource was denied [GDI+ status: {0}]", status);
			throw new UnauthorizedAccessException(text);
		}
		case Status.UnknownImageFormat:
		{
			string text = global::Locale.GetText("Either the image format is unknown or you don't have the required libraries to decode this format [GDI+ status: {0}]", status);
			throw new NotSupportedException(text);
		}
		case Status.NotImplemented:
		{
			string text = global::Locale.GetText("The requested feature is not implemented [GDI+ status: {0}]", status);
			throw new NotImplementedException(text);
		}
		case Status.WrongState:
		{
			string text = global::Locale.GetText("Object is not in a state that can allow this operation [GDI+ status: {0}]", status);
			throw new ArgumentException(text);
		}
		case Status.FontFamilyNotFound:
		{
			string text = global::Locale.GetText("The requested FontFamily could not be found [GDI+ status: {0}]", status);
			throw new ArgumentException(text);
		}
		case Status.ValueOverflow:
		{
			string text = global::Locale.GetText("Argument is out of range [GDI+ status: {0}]", status);
			throw new OverflowException(text);
		}
		case Status.Win32Error:
		{
			string text = global::Locale.GetText("The operation is invalid [GDI+ status: {0}]", status);
			throw new InvalidOperationException(text);
		}
		default:
		{
			string text = global::Locale.GetText("Unknown Error [GDI+ status: {0}]", status);
			throw new Exception(text);
		}
		}
	}
