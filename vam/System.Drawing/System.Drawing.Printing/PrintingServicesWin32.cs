...
internal class PrintingServicesWin32 : PrintingServices
{
	...
	internal static PrinterSettings.StringCollection InstalledPrinters
	{
		get
		{
			...
			try
			{
				...
				{
					PRINTER_INFO pRINTER_INFO = (PRINTER_INFO)Marshal.PtrToStructure(ptr, typeof(PRINTER_INFO));
	...
	internal override void LoadPrinterSettings(string printer, PrinterSettings settings)
	{
		...
		zero = Win32CreateIC(null, printer, null, IntPtr.Zero);
		num = Win32GetDeviceCaps(zero, 2);
		...
		try
		{
			...
			if (num2 != -1)
			{
				...
				for (int i = 0; i < num2; i++)
				{
					...
					PaperKind kind = (PaperKind)Marshal.ReadInt16(ptr3);
	...
	private void LoadPrinterPaperSources(string printer, PrinterSettings settings)
	{
		...
		try
		{
			...
			if (num2 != -1)
			{
				...
				for (int i = 0; i < num2; i++)
				{
					string sourceName = Marshal.PtrToStringUni(ptr);
	...
	{
		int num = Win32StartPage(gr.Hdc);
		return num > 0;
	}

	internal static bool EndPage(GraphicsPrinter gr)
	{
		int num = Win32EndPage(gr.Hdc);
		return num > 0;
	}
	...
	private string GetPrinterStatusMsg(uint status)
	{
		...
		if ((status & 0x40000u) != 0)
		{
			text += "No toner; ";
		}
		if ((status & 0x80000u) != 0)
		{
			text += "Page punt; ";
		}
		if ((status & 0x100000u) != 0)
		{
			text += "User intervention; ";
		}
		if ((status & 0x200000u) != 0)
		{
			text += "Out of memory; ";
		}
		if ((status & 0x400000u) != 0)
		{
			text += "Door open; ";
		}
		if ((status & 0x800000u) != 0)
		{
			text += "Server unkown; ";
		}
		if ((status & 0x1000000u) != 0)
		{
			text += "Power save; ";
		}
		return text;
