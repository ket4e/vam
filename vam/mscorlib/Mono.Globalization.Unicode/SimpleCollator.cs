...
internal class SimpleCollator
{
	...
	{
		return (opt & CompareOptions.IgnoreWidth) != 0 || MSCompatUnicodeTable.IsHalfWidthKana((char)cp);
	}
	...
	{
		return MSCompatUnicodeTable.IsIgnorable(i, (byte)(1 + (((opt & CompareOptions.IgnoreSymbols) != 0) ? 2 : 0) + (((opt & CompareOptions.IgnoreNonSpace) != 0) ? 4 : 0)));
	}
	...
	private unsafe void GetSortKey(string s, int start, int end, SortKeyBuffer buf, CompareOptions opt)
	{
		...
		ClearBuffer(ptr, 4);
		Context context = new Context(opt, null, null, null, null, ptr, quickCheckPossible: false);
	...
	private int CompareOrdinal(string s1, int idx1, int len1, string s2, int idx2, int len2)
	{
		...
		{
			throw new SystemException($"CompareInfo Internal Error: Should not happen. {idx1} {idx2} {len1} {len2} {s1.Length} {s2.Length}");
		}
		int num4 = idx1;
		int num5 = idx2;
	...
	public unsafe int IndexOf(string s, string target, int start, int length, CompareOptions opt)
	{
		...
		ClearBuffer(ptr4, 4);
		ClearBuffer(ptr5, 4);
		Context ctx = new Context(opt, ptr, ptr2, ptr4, ptr5, null, quickCheckPossible: false);
