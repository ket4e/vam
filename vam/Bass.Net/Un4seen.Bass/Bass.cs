...
public sealed class Bass
{
	...
	{
		IntPtr intPtr = BASS_ChannelGetTags(handle, BASSTag.BASS_TAG_ID3V2);
		if (intPtr != IntPtr.Zero)
		{
			try
			{
	...
	public static bool BASS_FXGetParameters(int handle, object par)
	{
		lock (_mutex)
		{
			bool result = BASS_FXGetParametersExt(handle, par);
			if (par is BASS_BFX_MIX)
			{
