...
internal class TermInfoDriver : IConsoleDriver
{
	...
	private static string[] locations = new string[3] { "/etc/terminfo", "/usr/share/terminfo", "/usr/lib/terminfo" };
	...
	private static TermInfoStrings[] UsedKeys = new TermInfoStrings[60]
	{
	...
	public ConsoleColor BackgroundColor
	{
		...
		set
		{
			...
			bgcolor = value;
			bool light;
			int num = TranslateColor(value, out light);
			if (light)
			{
				WriteConsole(string.Format(setlbgcolor, num));
			}
			else
			{
				WriteConsole(string.Format(setbgcolor, num));
			}
		}
	}
	...
	public ConsoleColor ForegroundColor
	{
		...
		set
		{
			...
			if (light)
			{
				WriteConsole(string.Format(setlfgcolor, num));
			}
			else
			{
				WriteConsole(string.Format(setfgcolor, num));
			}
		}
	}
	...
	public bool CursorVisible
	{
		...
		set
		{
			...
			cursorVisible = value;
			WriteConsole((!value) ? csrInvisible : csrVisible);
		}
	}
	...
	private static string SearchTerminfo(string term)
	{
		...
		foreach (string text in array)
		{
			...
			string path = Path.Combine(text, term.Substring(0, 1));
			if (Directory.Exists(text))
			{
	...
	public unsafe void Init()
	{
		...
		lock (initLock)
		{
			...
			setlfgcolor = ((!color16) ? setfgcolor : setfgcolor.Replace("[3", "[9"));
			setlbgcolor = ((!color16) ? setbgcolor : setbgcolor.Replace("[4", "[10"));
			string text2 = ((origColors != null) ? origColors : origPair);
			if (text2 != null)
			{
				text += text2;
			}
			...
			{
				string text3 = cursorAddress.Replace("%i", string.Empty);
				home_1_1 = cursorAddress != text3;
				cursorAddress = MangleParameters(text3);
			}
			GetCursorPosition();
			if (noGetPosition)
			{
	...
	public string ReadLine()
	{
		...
		do
		{
			key = ReadKeyInternal(out var fresh);
			flag = flag || fresh;
	...
	public void ResetColor()
	{
		...
		string str = ((origPair == null) ? origColors : origPair);
		WriteConsole(str);
	}
	...
	public void SetCursorPosition(int left, int top)
	{
		...
		int num = (home_1_1 ? 1 : 0);
		WriteConsole(string.Format(cursorAddress, top + num, left + num));
		cursorLeft = left;
		cursorTop = top;
	}
	...
	private void InitKeys()
	{
		if (!initKeys)
		{
			...
			TermInfoStrings[] usedKeys = UsedKeys;
			foreach (TermInfoStrings s in usedKeys)
			{
				AddStringMapping(s);
			}
