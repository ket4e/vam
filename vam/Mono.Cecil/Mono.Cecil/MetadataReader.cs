	...
	public Collection<CustomDebugInformation> GetCustomDebugInformation(ICustomDebugInformationProvider provider)
	{
		...
		for (int i = 0; i < value.Length; i++)
		{
			...
			else if (value[i].Col1 == EmbeddedSourceDebugInformation.KindIdentifier)
			{
				...
				{
					MemoryStream compressedStream = new MemoryStream(signatureReader3.ReadBytes((int)num3));
					byte[] content = new byte[num2];
					MemoryStream target = new MemoryStream(content);
					using (DeflateStream self = new DeflateStream(compressedStream, CompressionMode.Decompress, leaveOpen: true))
					{
						self.CopyTo(target);
					}
