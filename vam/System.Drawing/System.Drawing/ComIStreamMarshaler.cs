...
internal sealed class ComIStreamMarshaler : ICustomMarshaler
{
	...
	private sealed class ManagedToNativeWrapper
	{
		...
		static ManagedToNativeWrapper()
		{
			...
			exceptionGetHResult = typeof(Exception).GetProperty("HResult", BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.GetProperty | BindingFlags.ExactBinding, null, typeof(int), new Type[0], null).GetGetMethod(nonPublic: true);
		...
		internal static void ReleaseInterface(IntPtr comInterface)
		{
			if (comInterface != IntPtr.Zero)
			{
				...
				ReleaseSlot releaseSlot = (ReleaseSlot)Marshal.PtrToStructure((IntPtr)((long)intPtr + IntPtr.Size * 2), typeof(ReleaseSlot));
	...
	private delegate int ReadDelegate(IntPtr @this, [Out][MarshalAs(UnmanagedType.LPArray, SizeConst = 0, SizeParamIndex = 2)] byte[] pv, int cb, IntPtr pcbRead);
