...
public sealed class Utils
{
	...
	{
		return ((long)highWord << 32) | (lowWord & 0xFFFFFFFFu);
	}

	public static long MakeLong64(long lowWord, long highWord)
	{
		return (highWord << 32) | (lowWord & 0xFFFFFFFFu);
	}
	...
	{
		return (int)((qWord >> 32) & 0xFFFFFFFFu);
	}
	...
	public static string FixTimespan(double seconds, string format)
	{
		...
		try
		{
			...
			{
				"HHMM" => dateTime.ToString("HH:mm"), 
				"HHMMSS" => dateTime.ToString("HH:mm:ss"), 
				"MMSS" => dateTime.ToString("mm:ss"), 
				"MMSSFFF" => dateTime.ToString("mm:ss.fff"), 
				"MMSSFF" => dateTime.ToString("mm:ss.ff"), 
				"MMSSF" => dateTime.ToString("mm:ss.f"), 
				"HHMMSSFFF" => dateTime.ToString("HH:mm:ss.fff"), 
				"HHMMSSFF" => dateTime.ToString("HH:mm:ss.ff"), 
				"HHMMSSF" => dateTime.ToString("HH:mm:ss.f"), 
				"SMPTE" => dateTime.ToString("HH.mm.ss.f"), 
				"SMPTE24" => $"{dateTime.Hour}:{dateTime.Minute}:{dateTime.Second}:{Math.Round((double)dateTime.Millisecond / 41.666666666666664):F0}", 
				"SMPTE25" => $"{dateTime.Hour}:{dateTime.Minute}:{dateTime.Second}:{Math.Round((double)dateTime.Millisecond / 40.0):F0}", 
				"SMPTE30" => $"{dateTime.Hour}:{dateTime.Minute}:{dateTime.Second}:{Math.Round((double)dateTime.Millisecond / 33.333333333333336):F0}", 
				"SMPTE50" => $"{dateTime.Hour}:{dateTime.Minute}:{dateTime.Second}:{Math.Round((double)dateTime.Millisecond / 20.0):F0}", 
				"SMPTE60" => $"{dateTime.Hour}:{dateTime.Minute}:{dateTime.Second}:{Math.Round((double)dateTime.Millisecond / 16.666666666666668):F0}", 
