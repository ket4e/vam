...
public sealed class BassDShow
{
	...
	public static bool BASS_DSHOW_Init(IntPtr hWnd, BASSDSHOWInit flags)
	{
		...
		return BASS_DSHOW_Init(hWnd, (int)flags);
	}
	...
	private static extern int BASS_DSHOW_StreamCreateFileUnicode([In][MarshalAs(UnmanagedType.LPWStr)] string file, int pos, IntPtr win, BASSFlag flags);

	public static int BASS_DSHOW_StreamCreateFile(string file, int pos, IntPtr win, BASSFlag flags)
	{
		flags |= BASSFlag.BASS_UNICODE;
		return BASS_DSHOW_StreamCreateFileUnicode(file, pos, win, flags);
	}
	...
	private static extern int BASS_DSHOW_StreamCreateFilterUnicode([In][MarshalAs(UnmanagedType.LPWStr)] string file, [In][MarshalAs(UnmanagedType.LPStr)] string filter, IntPtr win, BASSFlag flags);

	public static int BASS_DSHOW_StreamCreateFilter(string file, string filter, IntPtr win, BASSFlag flags)
	{
		flags |= BASSFlag.BASS_UNICODE;
		return BASS_DSHOW_StreamCreateFilterUnicode(file, filter, win, flags);
	}
	...
	{
		BASS_DSHOW_CHANNELINFO info = new BASS_DSHOW_CHANNELINFO();
		BASS_DSHOW_ChannelGetInfo(handle, ref info);
		return info;
	}
	...
	public static extern void BASS_DSHOW_ChannelSetFullscreen(int handle, [In][MarshalAs(UnmanagedType.Bool)] bool fullScreen);
	...
	{
		IntPtr intPtr = BASS_DSHOW_ChannelGetBitmapPtr(handle);
	...
	{
		BASS_DSHOW_COLORRANGE ctrl = new BASS_DSHOW_COLORRANGE();
		if (BASS_DSHOW_ChannelColorRange(handle, id, ref ctrl))
		{
			return ctrl;
		}
		return null;
	...
	[return: MarshalAs(UnmanagedType.Bool)]
	private static extern bool BASS_DSHOW_ChannelAddFileUnicode(int handle, [In][MarshalAs(UnmanagedType.LPWStr)] string filename, BASSFlag flags);

	public static bool BASS_DSHOW_ChannelAddFile(int handle, string filename)
	{
		return BASS_DSHOW_ChannelAddFileUnicode(handle, filename, BASSFlag.BASS_UNICODE);
	}
	...
	private static extern int BASS_DSHOW_LoadPluginUnicode([In][MarshalAs(UnmanagedType.LPWStr)] string file, BASSFlag flags);

	public static int BASS_DSHOW_LoadPlugin(string file)
	{
		return BASS_DSHOW_LoadPluginUnicode(file, BASSFlag.BASS_UNICODE);
	}

	[DllImport("xVideo", EntryPoint = "xVideo_LoadPluginDS")]
	private static extern int BASS_DSHOW_LoadPluginDSUnicode([In][MarshalAs(UnmanagedType.LPWStr)] string guid, [In][MarshalAs(UnmanagedType.LPWStr)] string name, BASSFlag flags);

	public static int BASS_DSHOW_LoadPluginDS(string guid, string name)
	{
		return BASS_DSHOW_LoadPluginDSUnicode(guid, name, BASSFlag.BASS_UNICODE);
	}
	...
	{
		bool num = BASS_DSHOW_PluginGetInfoInternal(plugin, ref info._internal);
		if (num)
		{
	...
	public BASS_DSHOW_PLUGININFO BASS_DSHOW_PluginGetInfo(int plugin)
	{
		...
		if (BASS_DSHOW_PluginGetInfo(plugin, bASS_DSHOW_PLUGININFO))
		{
	...
	private static extern int BASS_DSHOW_StreamCreateDVDUnicode([In][MarshalAs(UnmanagedType.LPWStr)] string dvd, IntPtr win, BASSFlag flags);

	public static int BASS_DSHOW_StreamCreateDVD(string dvd, IntPtr win, BASSFlag flags)
	{
		flags |= BASSFlag.BASS_UNICODE;
		return BASS_DSHOW_StreamCreateDVDUnicode(dvd, win, flags);
	}
	...
	[return: MarshalAs(UnmanagedType.Bool)]
	private static extern bool BASS_DSHOW_RegisterUnicode([In][MarshalAs(UnmanagedType.LPWStr)] string email, [In][MarshalAs(UnmanagedType.LPWStr)] string reg, BASSFlag flags);

	public static bool BASS_DSHOW_Register(string email, string reg)
	{
		return BASS_DSHOW_RegisterUnicode(email, reg, BASSFlag.BASS_UNICODE);
	}

	[DllImport("xVideo", EntryPoint = "xVideo_VCamStreamChannel")]
	[return: MarshalAs(UnmanagedType.Bool)]
	public static extern bool BASS_DSHOW_VCamStreamChannel(int handle, [In][MarshalAs(UnmanagedType.Bool)] bool stream);
	...
	public static BASS_DSHOW_STREAMS BASS_DSHOW_ChannelGetStream(int handle, int index)
	{
		...
		if (BASS_DSHOW_ChannelGetStream(handle, index, ref bASS_DSHOW_STREAMS._internal))
		{
	...
	{
		return Utils.LoadLib("xVideo", ref _myModuleHandle);
	}

	public static bool LoadMe(string path)
	{
		return Utils.LoadLib(Path.Combine(path, "xVideo"), ref _myModuleHandle);
	}
	...
	private static void CheckVersion()
	{
		try
		{
			if (Utils.HighWord(BASS_DSHOW_GetVersion()) == 258)
			{
				return;
			}
			Version version = BASS_DSHOW_GetVersion(2);
